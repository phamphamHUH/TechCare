  ...
  const handleDropOnCanvas = (e: React.DragEvent, targetIndex?: number) => {
    ...
    if (rawData) {
      try {
        ...
        // Not a JSON palette drag
      }
    }

    // Canvas-to-canvas reordering
  ...
  return (
    <div className="flex-1 min-w-0 bg-white border border-dashed border-sky-300 rounded-3xl p-6 sm:p-8 flex flex-col gap-6 shadow-sm overflow-y-auto max-h-[calc(100vh-140px)]">
      ...
      <div
        ...
        className={`flex flex-wrap gap-4 min-h-[160px] p-2 rounded-2xl transition-all ${
          isPaletteOverCanvas ? "bg-sky-50/50 border-2 border-dashed border-sky-400" : ""
        }`}
      >
        {components.length === 0 ? (
          <div className="w-full py-12 flex flex-col items-center justify-center text-center text-gray-400">
            <p className="text-sm font-medium">No components added yet</p>
            <p className="text-xs text-gray-400 mt-0.5">
              Click or drag fields from the left palette to begin building your report.
            </p>
          </div>
        ) : (
      ...
      <div
        ...
        onDragLeave={handleDragLeaveCanvas}
        onDrop={(e) => handleDropOnCanvas(e)}
        className="w-full py-3.5 border-2 border-dashed border-sky-300 rounded-2xl bg-sky-50/60 hover:bg-sky-50 flex items-center justify-center text-xs font-bold text-sky-600 transition-colors select-none"
      >
